...
export default function CategoryPage(){
    ...
    const [routineContainer,setRoutineContainer] = useState('');
    const makeRoutineContainer = useRef(null);
    const makeRoutineBox =()=>{
        const li = document.createElement('button'); 
        li.innerHTML = `<h1>새로운 루틴 1</h1>`;
       document.querySelector(`.container.mt-0.p-0`).appendChild(li);
    }
    useEffect(()=>{
        console.log(document.querySelector(`.container.mt-0.p-0`));
    },[])
    return<>
        <div className={"container mt-5 p-3"}></div>
            ...
            <div ref={makeRoutineContainer} className={`${styles.mainpage} container mt-0 p-0`}>
